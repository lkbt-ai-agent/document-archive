...
PostgreSQL 버전 확인 필요 DB가 주 저장소입니다. pgvector extension 버전 확인 필요 항목은 `DocumentChunk.embedding` 벡터를 같은 DB 안에 저장합니다. 원본/생성 파일 본문은 DB가 아니라 로컬 디스크 또는 MinIO server 버전 확인 필요 저장소에 저장하고, DB에는 위치만 기록합니다.

테이블은 SQLAlchemy 2 모델에서 생성합니다. Pydantic 2 schema는 API 입출력 직렬화에 사용합니다.
...
저장소: PostgreSQL `document_chunks`. `embedding`은 pgvector extension 컬럼입니다.
...
현재 모델은 테이블과 pgvector extension을 생성하지만 별도 관계형 인덱스나 HNSW 인덱스를 정의하지 않습니다. 데이터가 커지면 다음 인덱스가 필요합니다.
